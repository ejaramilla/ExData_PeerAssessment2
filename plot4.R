...
NEISCC.filtered <- NEISCC.filtered[grep("^Fuel Comb.*Coal$",NEISCC.filtered$EI.Sector, ignore.case=TRUE, perl=TRUE), ]
...
resultGraph <- ggplot(graphData, aes(year, Emissions, fill=Sectors)) +
  ...
  ggtitle("Total PM2.5 Emission from Coal Combustion")

ggsave(resultGraph, file="plot4.png")
