...
NEISCC.filtered <- NEISCC.filtered[NEISCC.filtered$fips == "24510" | NEISCC.filtered$fips == "06037", ]
NEISCC.filtered <- NEISCC.filtered[grep("^Mobile",NEISCC.filtered$EI.Sector, ignore.case=TRUE, perl=TRUE), ]
graphData <- aggregate(NEISCC.filtered$Emissions, by=list(NEISCC.filtered$fips, NEISCC.filtered$year, NEISCC.filtered$EI.Sector), FUN=sum)
names(graphData) <- c("fips","year","Sectors","Emissions")
graphData$Region <- ifelse(graphData$fips == "24510","Baltimore City","Los Angeles County")
...
resultGraph <- ggplot(graphData, aes(year, Emissions, fill=Sectors)) +
  ...
  ggtitle("PM2.5 Emission from Motor Vehicles in\nBaltimore City vs Los Angeles County")

ggsave(resultGraph, file="plot6.png")
